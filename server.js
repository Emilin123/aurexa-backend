...
async function lootLockerSession() {
 ...
 const { data } = await axios.post(`${LOOTLOCKER_BASE}/session`, { game_version: '1.0.0', game_id: LOOTLOCKER_GAME_ID }, { headers: { 'x-server-key': LOOTLOCKER_SERVER_KEY, 'LL-Version': '2021-03-01', 'Content-Type': 'application/json' } });
 if (!data?.token) throw new Error('LootLocker did not return a server session token'); return data.token;
}
async function lootLockerPlayerForFirebase(firebaseUid) {
 const { data, error } = await supabase.from('lootlocker_player_mappings').select('lootlocker_player_id').eq('firebase_uid', firebaseUid).eq('game_id', LOOTLOCKER_GAME_ID).maybeSingle();
 if (error) throw new Error('No se pudo consultar la asociación LootLocker');
 return data?.lootlocker_player_id ? String(data.lootlocker_player_id) : null;
}
...
app.post('/api/solicitar-compra', requireFirebaseUser, async (req, res) => {
 try {
 const { cup, monto, amount, tokens, tokens_requested, full_name, whatsapp, package_code, payment_reference } = req.body || {};
 const lootlockerPlayerId = await lootLockerPlayerForFirebase(req.firebaseUser.uid);
 if (!lootlockerPlayerId) return res.status(409).json({ ok: false, error: 'La cuenta aún no tiene una asociación LootLocker verificada' });
