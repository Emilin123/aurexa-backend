...
// Server-authoritative Firebase/Firestore routes. These never trust client time or balance.
...
function getFallbackBotReply(userQuery: string): string {
  ...
  if (q.includes('minar') || q.includes('mina') || q.includes('pozo')) {
    return '⛏️ Para minar, abre “Empezar a Minar”. Las liquidaciones de producción se validan en el servidor y no dependen del reloj del teléfono.';
  }
  if (q.includes('carta') || q.includes('rifa') || q.includes('sorteo')) {
    return '🃏 Carta Ganadora está disponible según el horario publicado en la aplicación. Las participaciones válidas deben ser confirmadas por el servidor.';
  }
  if (q.includes('retir') || q.includes('billetera')) {
    return '💰 Tu billetera muestra el saldo confirmado por Aurexa. Los retiros se habilitan únicamente cuando la operación backend correspondiente está disponible.';
  }
  return '🏰 Soy AurexaBot, tu guía. Puedo orientarte sobre minería, billetera, compras, soporte y Carta Ganadora. No tengo acceso a información administrativa privada.';
}
...
app.post('/api/assistant/chat', async (req, res) => {
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      console.warn('[Gemini Assistant Error] Fallback:', err.message);
    }
  }
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    services: { backend: 'online', firebase: Boolean(process.env.FIREBASE_SERVICE_ACCOUNT_JSON), telegramBot: { configured: Boolean(TELEGRAM_BOT_TOKEN), webhookSecretSet: Boolean(TELEGRAM_WEBHOOK_SECRET) }, render: 'online' },
  });
});
...
app.post('/api/telegram/notify', async (req, res) => {
  ...
  if (eventId && processedEventIds.has(eventId)) return res.json({ success: true, deduplicated: true });
  ...
  try {
    ...
    return res.status(503).json({ success: false, error: 'Telegram API no disponible' });
  }
...
app.post('/api/telegram/webhook', async (req, res) => {
  ...
  const replyText = cmd === '/start' ? '👑 Sistema administrativo de Aurexa activo.' : cmd === '/id' ? `🆔 Chat autorizado: \`${chatId}\`` : '📖 Comandos administrativos disponibles en el canal privado.';
  if (TELEGRAM_BOT_TOKEN) {
    try { await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text: replyText, parse_mode: 'Markdown' }) }); } catch {}
  }
